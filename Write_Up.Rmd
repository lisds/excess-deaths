...
excess_mortality.sort_values('Mean 2023').head()
```
...
To allow for population size we needed to calculate the proportion of indidividuals in countries that had been vaccinated.
...
We will now check whether linear regression is appropriate technique to investigate the relationship between percentage vaccination up take pre 2023 and mean excess mortality in 2023.

** dont need to reference causality yet will do that later
...
plt.ylabel('Residuals')
plt.title('Residual vs. Regressor Plot (for vaccination regressor)')
plt.show()
```
...
plt.xlabel('Regressor Values')
plt.ylabel('Residuals')
plt.title('Residual vs. Regressor Plot (for mortality regressor)')
plt.show()
```
...
Our vaccination data includes the weekly number of first doses by each vaccination type (for each country). Different vaccine manufacturures produce different kinds of vaccines. Pfizer-BioTech and Moderna are both mRNA vaccines, which are are supposedly linked to an increased number of cardiac incidents (Paknahad et.al, 2023). Both the Oxford Astrazeneca and Janssen (Johnson and Johnson) vaccines are viral vector vaccines. It is resonable to suggets that each vaccine type could have a different impact on the health of individuals, and therefore excess mortality. In addition, each manufacturer will have their own unique process of production. 

We first investigated the relationship between excess mortality and vaccine by manufacturer. Using the same mean excess mortality figure from our first regresssion, we conducted a linear regression for each maunfacturer.
...
for x, y, country in zip(vaccine_dict['Janssen']['% vaxed'],
    ...
    axs[1, 1].set_title('Janssen')

for ax in axs.flat:
    ax.set(xlabel='Percentage Vaccinated', ylabel='Mean Excess Death 2023')
...
The p-value and r-values from the linregress function assume linearity and independence of the given variables. Unfortunately, due to the highly interconnected value of mortality which can be affected by a variety of factors and the fact that the vaccination rate itself is affected by death rates it is un-wise to take those values at face value.
...
filtered_array = [value for value in order_array if value in pie_chart_data.index]
