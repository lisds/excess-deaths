...
Running the following line of code will fetch the vaccination data we will use in our project. Due to an issue of obtaining direct download links for the excess mortality data, please refer to the README.md file for further instructions before proceeding with the rest of this notebook.
...
However, we have not considered for any external factors as yet in our analysis. We cannot be confident that other factors are not influencing our regression models as we haven't accounted for them. Indentifying these factors would help us in attempting to isolate the impact of vaccines from those factors.

Such factors which could be influencing the results we are seeing are as follows: 

- Strength of Countries' Healthcare Systems: Better healthcare could lead to lower mortality rates.
- Age demographic of countries: Countries' with aging populations could tend towards certain vaccine types (for various reasons) which could make those vaccines appear to be influencing excess mortality. For example, countries with an aging population may be more affluent and opted for more expensive vaccines. 
- Influenza Rates: If a country is also dealing with a flu outbreak, it could affect mortality rates.
- Lockdown effects: Countries with stricter vaccination protocols seem likely to have had harsher lockdown restrictions. Strict lockdowns might have indirect health impacts, like increased alcohol consumption leading to more liver-related deaths (observed in the UK CITE)


It will also be useful to explore any commonalities between the groupings of countries. This may help us indentify any further or more specific factors to be aware of: Which countries had similar vaccine profiles? Do they have other common traits? 
Common traits may come in the form of - 
...
We will return to an initial histogram of mean excess all cause mortality to begin to investigate the impact external factors to our research could have had on our initial findings.
...
##### Strength of Countries' Healthcare Systems:
...
mean_2023_mortality.sort_values(by = 'p_proj_all_ages', ascending = False)
```
...
Did certain countries lock down more than others? have they had more deaths?
