...
It is important to note that excess death statistics have not previously been recorded at the same scale as those for the Covid-19 pandemic. As a result, it is difficult to compare contemporary and historical data.
...
The dataset contains many columns we are not interested in we remove those columns

```{python}
#Dropping unwanted columns
base_vaccine_data = base_vaccine_data.drop(['Denominator', 'NumberDosesReceived', 'NumberDosesExported', 'FirstDoseRefused', 'DoseAdditional1', 'DoseAdditional2', 'DoseAdditional3', 'DoseAdditional4', 'DoseAdditional5', 'UnknownDose'], axis = 1)
base_vaccine_data
```
...
```{python}
base_vaccine_data['ReportingCountry'].value_counts()
```
...
```{python}
base_vaccine_data.groupby('Vaccine')['FirstDose'].sum()
```

#### Descriptive statistics


- Maybe say what one row in each cleaned dataset represents? - granularity of the dataset
- Kessia to add here descriptive stats of two datasets 
- What is important to contextualise the data - mean excess mortality across countries, max and min mortality and vaccination rate etc
...
base_vaccine_data.groupby('Year')['FirstDose'].sum()
```

```{python}
#Displaying counts of second vaccination dose by year
base_vaccine_data.groupby('Year')['SecondDose'].sum()
```
...
excess_deaths_values['Mean 2023'] = excess_deaths_values[months_2023].apply(pd.to_numeric, errors='coerce').mean(axis = 1)
...
- When explain merge highlight the fact that two of the columns from the excess mortality dataframe are lost 
<!-- #endregion -->
